...
remove_arch() {
    ...
    sudo pacman -R --noconfirm python3-pip
    sudo pacman -R --noconfirm g++
    sudo pacman -R --noconfirm python3
}
...
remove_mingw64() {
    ...
    pacman -R --noconfirm python-pip
    pacman -R --noconfirm g++
    pacman -R --noconfirm python
}
...
options_mingw64(){
    read -p "Do you when to uninstall: ALL - press A, decide one by one - press D, cancel - press C: " option
    if [  "$option" = "A" -o "$option" = "a"  ]; then
        remove_mingw64
    elif [  "$option" = "D" -o "$option" = "d"  ]; then
