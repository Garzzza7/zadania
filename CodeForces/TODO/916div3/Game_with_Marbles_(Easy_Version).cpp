...
int main() {
  ...
  while (T--) {
    ...
    /*sort(a.begin(), a.end(), pair_cmp);*/
    /*sort(b.begin(), b.end(), pair_cmp);*/
    ...
    for (int i = 0; i < n; i++) {
      if (swt) {
        B--;
        int iter = 0;
        while (done.contains(a[iter].second)) {
          iter++;
        }
        done.insert(a[iter].second);
        A -= a[iter].first;
      } else {
        A--;
        int iter = 0;
        while (done.contains(b[iter].second)) {
          iter++;
        }
        done.insert(b[iter].second);
        B -= b[iter].first;
      }
      swt ^= 1;
    }
