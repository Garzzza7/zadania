...
struct tarjan_find_articulation {
    ...
    std::vector<char> visited, is_art_point;
    ...
    void dfs(const int p, const int v) {
        ...
        for (const auto &e : adj[v]) {
            ...
            if (visited[e]) {
                ...
                if (low[e] >= entry_time[v] and p != -123) { is_art_point[v] = true; }
            }
        }
        if (p == -123 and decs > 1) { is_art_point[v] = true; }
    }

    void run(void) {
        for (int i = 0; i < static_cast<int>(visited.size()); i++) {
            if (not visited[i]) { dfs(-123, i); }
        }
    }

    void print(void) {
        for (const auto &ee : adj) {
            for (const auto &e : ee) std::cout << e << " ";
            std::cout << "\n";
        }
    }
};
...
int main(void) {
    ...
    for (int i = 0; i < static_cast<int>(graph.is_art_point.size()); i++)
        if (graph.is_art_point[i]) std::cout << i << " is an articulation point.\n";
