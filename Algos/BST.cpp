...
template <typename T = int> struct bst {
	...
	std::cout << "pred of " << n->val << " = "
		  << (this->find_predecessor(n) == nullptr
			  ? -69
			  : this->find_predecessor(n)->val)
		  << "\n";
    }
    void
    disp_succ(node<T> *n) {
	std::cout << "succ of " << n->val << " = "
		  << (this->find_successor(n) == nullptr
			  ? -69
			  : this->find_successor(n)->val)
		  << "\n";
    }
};
