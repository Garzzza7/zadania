...
void dfs(const int parent, const int vertex, std::vector<std::vector<int>> &adj,
         std::vector<std::vector<int>> &ancestors, std::vector<int> &depth, int &limit) {
    if (parent != vertex) {
    ...
    for (const auto &ver : adj[vertex]) {
        if (vertex != ver) { dfs(vertex, ver, adj, ancestors, depth, limit); }
    }
}
...
int query(const int vertex, const int kth, const std::vector<int> &depth,
    ...
    int ancestor{vertex};
    if (depth[vertex] < kth or kth < 0) { return -123; }
    auto is_on{[](const int &a, const int &b) -> bool { return a & (1 << b); }};
    for (int comb = 0; comb < limit; comb++) {
        if (is_on(kth, comb)) { ancestor = ancestors[ancestor][comb]; }
    }
    return ancestor;
}
