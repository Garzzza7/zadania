...
if [[ ! "$(./Pollard_Rho.sol < Pollard_Rho.txt)" ]]; then
    printf "${red}ABORT at Pollard_Rho.${normal}\n"
    cnt_aborted=$((cnt_aborted + 1))
elif [ "$(./Pollard_Rho.sol < Pollard_Rho.txt)" == "$(cat Pollard_Rho.test)" ]; then
    printf "${green}Pollard_Rho Passed.${normal}\n"
    cnt_passed=$((cnt_passed + 1))
else
    printf "${red}Pollard_Rho Failed.\n"
    printf "${red} Got:\n"
    printf "${red}$(./Pollard_Rho.sol < Pollard_Rho.txt)\n"
    printf "${red} Should be:\n"
    printf "${red}$(cat Pollard_Rho.test)${normal}\n"
    cnt_failed=$((cnt_failed + 1))
fi
...
if [[ ! "$(./Sum_of_kth_sized_subsets.sol < Sum_of_kth_sized_subsets.txt)" ]]; then
    printf "${red}ABORT at Sum_of_kth_sized_subsets.${normal}\n"
    cnt_aborted=$((cnt_aborted + 1))
elif [ "$(./Sum_of_kth_sized_subsets.sol < Sum_of_kth_sized_subsets.txt)" == "$(cat Sum_of_kth_sized_subsets.test)" ]; then
    printf "${green}Sum_of_kth_sized_subsets Passed.${normal}\n"
    cnt_passed=$((cnt_passed + 1))
else
    printf "${red}Sum_of_kth_sized_subsets Failed.\n"
    printf "${red} Got:\n"
    printf "${red}$(./Sum_of_kth_sized_subsets.sol < Sum_of_kth_sized_subsets.txt)\n"
    printf "${red} Should be:\n"
    printf "${red}$(cat Sum_of_kth_sized_subsets.test)${normal}\n"
    cnt_failed=$((cnt_failed + 1))
fi
