...
convex_hull(std::vector<point<T>> points) {
    ...
    auto check = [](const point<T> &curr, const point<T> &l,
		    const point<T> &r) -> int {
	return (l.x - curr.x) * (r.y - curr.y)
	       - (l.y - curr.y) * (r.x - curr.x);
    };

    std::sort(points.begin(), points.end(),
	      [](const point<T> &l, const point<T> &r) {
		  if (l.x != r.x)
		      return l.x < r.x;
		  return l.y < r.y;
	      });
    ...
    for (int i = 0; i < n; i++) {
	while ((int) low.size() >= 2
	       && check(low[(int) low.size() - 2], low[(int) low.size() - 1],
			points[i])
		      <= 0)
	    low.pop_back();
    ...
    for (int i = n - 1; i > 0; i--) {
	while ((int) up.size() >= 2
	       && check(up[(int) up.size() - 2], up[(int) up.size() - 1],
			points[i - 1])
		      <= 0)
	    up.pop_back();
